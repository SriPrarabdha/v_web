...
export async function POST(req: NextRequest) {
  try {
    ...
    const hashString = `${merchantKey}|${formData.txnid}|${formData.amount}|${formData.productinfo || 'None'}|${formData.firstname || 'Customer'}|${formData.email || 'customer@example.com'}|||||||||||${salt}`
